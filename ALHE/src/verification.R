...
verifyFunctionToLoad = function() {
    ...
    if(ok == FALSE) {
        loggerERROR("Blad w definicji funkcji. Popraw ja i uruchom skrypt ponownie.");
        quit("ask");
    }
}
...
verifyLoadedFunction = function() {
    ok = TRUE;
    if(is.null(functionName)) {ok = FALSE; loggerERROR("Brak parametru: 'functionName'");}
    ...
    if(is.null(optimum)) {loggerERROR("Brak parametru: 'optimum'. Wlaczam tryb wykrywania najlepszego punktu.");}
    if(is.null(optimumValue)) {loggerERROR("Brak parametru: 'optimumValue'. Wlaczam tryb wykrywania najlepszego punktu.");}
    ...
    if(!is.null(better)) {
        if(better != "min" && better != "max") {ok = FALSE; loggerERROR(paste("Parametr 'better' ma niedozwolona wartosc: [", better, "], dozwolone sa: [min, max]"));}
    }
